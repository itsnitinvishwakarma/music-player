...
class view:
    def __init__(self, top=None):
        ...
        self.songName.configure(text='''Apna time ayga''')
    ...
    def play_song(self):
        ...
        print("thread status=",self.isThreadRunning)
        ...
        try:
            ...
            print("pause status=",self.isPaused)
            ...
            messagebox.showerror("selection Error",ex1)
    ...
    def remove_song(self):
        ...
        try:
            if len(self.sel_index_tuple) == 0:
                raise NoSongSelectedError("plz select a song to remove from playlist")
    ...
    def close_window(self):
        result=messagebox.askyesno("App closing!!!","r u want to quit")
    ...
    def load_previous_song(self):
        try:
            if hasattr(self,"sel_song_index_tuple")==False:
                raise NoSongSelectedError("plz play a song first")
...
#            # prev_song_index = self.cur_song_index[0]
...
#            self.show_song_details()
          ...
          #  self.change_volume(self.vol_scale.get())
           # self.isPlaying = True
            #self.song_name = self.playList.get(self.cur_song_index[0])
            #for x in self.playList.get(0,END):
             #   if self.song_name == x:
            #        self.song_name =
           # self.my_player.play_song()
    ...
    def add_song_to_favourites(self):
        ...
        try:
            if len(fav_song_index_tuple)==0:
                raise NoSongSelectedError("plz select a song to add to favourites")
            ...
            messagebox.showerror("db error!","song cant be added")
            print(ex2)
    ...
    def load_songs_from_favourites(self):
        try:
            ...
            if result.find("no songs present")!=-1:
                messagebox.showinfo("favourites empty!!!","NO songs in ur favourites")
                return
            ...
            print(mycolor)
            self.playList.configure(fg=mycolor)
            messagebox.showinfo("success!!!","list populated from ur favourites")

        except(DatabaseError)as ex1:
            messagebox.showerror("db error!","sorry! songs cant be loaded from favourites!!")
    ...
    def show_timer(self, total_sec):
        ...
        while curr_sec <= total_sec and mixer.music.get_busy():
            ...
            #curr_sec +=1
            self.songProgress.step(1  *   100 / total_sec)
            time.sleep(1)
            print("curr _sec is :", curr_sec, "total sec is:", total_sec)
    ...
    def load_next_song(self):
        curr_song_index=self.playList.curselection()
        next_song_index=curr_song_index[0]+1
        print("type=",type(next_song_index))
        print("size of list is",self.playList.count(),"type=",type(self.playList.count()))
        if next_song_index > self.playList.count()-1:
            next_song_index=0

        self.playList.select_clear(0,tk.END)
        self.playList.selection_set(next_song_index)
        print("next song index:",next_song_index)
        self.play_song()
